...
import {deviceState, newTopologyState, pageState} from "../States";
import {getHnsP2pTopologies, getP2pTopologies, P2pTopologies} from "./P2pTopologies";

const hnsTopologiesContext: {hnsTopologies: any[]} = {hnsTopologies: []};

function getHnsTopologies(callback:any) {
    get("hns-topologies", (responseData)=>{hnsTopologiesContext.hnsTopologies = responseData; callback();}, 5, undefined, "topologies");
}
                ...
                <ListGroup>
                    {
                        hnsTopologiesContext.hnsTopologies.map((topology) => (
                            <ListGroup.Item action onClick={() => {
                                ...
                                getHnsP2pTopologies(()=>{pageState.setPage(<P2pTopologies/>);});
                            }}>
                                {topology.name}
                            </ListGroup.Item>
                        ))
